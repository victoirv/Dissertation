...
\section{Solar Wind}
The solar wind is the largest driver of particle density in the magnetosphere \vinote{cite \cite{Temerin2002NewModelPredictionDst}? They say solar wind drives Dst, and Dst is coupled to magnetospheric density via the ring current?}, and as such is an important input for a plasmatrough model. Conditions such as magnetic field orientation and particle velocity are important considerations for whether the plasmasphere is expected to be compressed, saturated, or experiencing high variability. By adding these to a model that includes time delayed inputs, their effects can be accounted for and aid in categorizing the state of the plasmatrough.
...
\vinote{Include sources for CCMC model section?}

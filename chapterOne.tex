...
The inner magnetosphere is composed of many constituent parts, such as the plasmasphere, the ring current, and the radiation belts, all of which are shown in Figure \ref{fig:magnetosphereoverview}.

\begin{figure}[htp]
	\centering
	\includegraphics[scale=0.3]{{Figures/innermag-2.jpg}}
	\caption{Overview of inner magnetosphere \vinote{Make less ugly. Can't find citable creator of original. From https://www.nasa.gov/mission\_pages/sunearth/science/inner-mag-mos.html}}
...
The complex structure of the magnetosphere and plasmasphere leads to a number of distinct behaviors and processes such as ring currents, geomagnetic storms, and magnetospheric substorms, all of which are driven by the solar wind.

The Ring Current, one of the major magnetospheric currents shown in Figure \ref{RingCurrentFigure}, is formed when charged particles interact with the magnetosphere causing the electrons captured from the solar wind to drift one direction around the Earth while the positive ions drift the opposite direction. A schematic of this from a top-down perspective is shown in Figure \ref{fig:Gold1960DriftMotion}. With enough energetic particles drifting together, a current is generated that can significantly affect the Earth's magnetic field and is labeled the ring current. These particles in the current are injected into the magnetosphere via conditions that cause the solar wind's magnetic field to connect with that of Earth, which is often exacerbated by the surge of energy created by geomagnetic storms.
...
At the lowest layer of the ionosphere, photoionization of oxygen produces excess electrons that are transferred up \inote{How?} into the plasmasphere along magnetic field lines bringing energy and heat along with it. There is also an upward daily ``refilling"  flux \inote{Why and what is process} until a saturation point is reached, bringing the lower bounds of the plasmasphere into equilibrium with the upper ionosphere. This flux also typically reverses on the night side, sending electrons back down into the ionosphere.

During periods when the location of the plasmapause is quickly brought earthwards, or a new plasmapause is established, plasma outside the plasmapause is known to be magnetically convected outwards and sunwards \citep{ErosionRecoveryPlasmasphere}, termed ``eroding". \inote{Rewrite to be more clear} 
